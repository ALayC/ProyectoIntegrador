...
public class AppDbContext : DbContext
{
    ...
    public DbSet<InvitacionAuxiliar> InvitacionesAuxiliar => Set<InvitacionAuxiliar>();
 ...
 .WithOne(e => e.Usuario)
  ...
  .WithOne(e => e.Usuario)
        ...
        // ??????????????????????????????????????????????
        modelBuilder.Entity<DispositivoConfiable>(entity =>
        {
            entity.HasKey(e => e.Id);

            entity.HasOne(e => e.Usuario)
                .WithMany(e => e.DispositivosConfiables)
                .HasForeignKey(e => e.UsuarioId)
        ...
        // InvitacionAuxiliar
        // ??????????????????????????????????????????????
        modelBuilder.Entity<InvitacionAuxiliar>(entity =>
        {
            entity.HasKey(e => e.Id);
